...
def market_sentiment(market_df):
    ...
    if "ocf_to_rev" in df.columns and df["ocf_to_rev"].notna().sum()>0: mask = mask & (df["ocf_to_rev"]>0.08)
    if "goodwill_to_equity" in df.columns and df["goodwill_to_equity"].notna().sum()>0: mask = mask & (df["goodwill_to_equity"]<0.30)
...
def detect_kline_patterns(hist_data):
    ...
    latest = hist_data.iloc[-1]; prev = hist_data.iloc[-2]; prev2 = hist_data.iloc[-3] if len(hist_data)>=3 else None
    ...
    if prev2 is not None:
        if prev_pct>5 and prev2["涨跌幅"]>3 and pct<-3 and turnover>prev_turnover: patterns.append("⚠️头肩顶风险")
        if prev_pct<-5 and prev2["涨跌幅"]<-3 and pct>3 and turnover>prev_turnover: patterns.append("🔻头肩底雏形")
        if prev2["涨跌幅"]>5 and abs(prev_pct)<2 and pct<-5: patterns.append("🏝️岛型反转预警")
    ...
    if prev2 is not None and pct>0 and prev_pct>0 and prev2["涨跌幅"]>0 and turnover>5: patterns.append("⚠️倒三阳诱多")
...
if not market_open: st.sidebar.warning("🔴 休市中，数据为最近交易日")
...
if main_page == "📊 行情总览":
    ...
    if market.empty: st.error("行情数据不可用")
        ...
        c1.metric("上涨家数", up); c2.metric("下跌家数", down)
        c3.metric("平均涨跌", f"{market['涨跌幅'].mean():.2f}%")
        c4.metric("低PE股票", int(len(market[(market["市盈率"]>0)&(market["市盈率"]<20)])))
        limit_up = int((market["涨跌幅"]>=9.9).sum()); limit_down = int((market["涨跌幅"]<=-9.9).sum())
        c5,c6 = st.columns(2)
        c5.metric("涨停家数", limit_up); c6.metric("跌停家数", limit_down)
                ...
                if not market.empty:
                    ...
                    if not m.empty:
                        ...
                        if info.get("buy_price") and info["buy_price"]>0:
                            chg = (s["最新价"]-info["buy_price"])/info["buy_price"]*100; delta = f"盈亏{chg:+.2f}%"
                        ca.write(f"{s['名称']}({c})"); cb.write(f"{s['最新价']}元 {s['涨跌幅']}% {delta}")
